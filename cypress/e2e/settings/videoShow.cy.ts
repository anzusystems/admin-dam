...
describe(`Test video shows function, Env: ${CY.cfg}`,
  ...
  it('Edit video show', ()=>{
    ...
    cy.urlContains('/edit')
    cy.cardLoad()
    cy.get('.v-card-text').find('input').eq(0).clear().type(`${VIDEO_SHOW_TITLE}-edit`)
  ...
  it('Create episode', () =>{
    cy.visit(`/video-shows/${VIDEO_SHOW_ID}`)
    cy.getCy('episode-list').click()
    cy.getCy('button-create').click()
    cy.getCy('episode-title').find('input').eq(0).clear().type(`${EPISODE_TITLE}-edit`)
    cy.getCy('button-close').should('be.visible')
    cy.getCy('button-cancel').should('be.visible')
    cy.getCy('button-confirm').click()
    cy.alertMessage(ALERT_CREATE)
    ...
    cy.getCy('copy-text')
      ...
      .then((text) => {
        ...
        cy.getCyVisibleClick('button-close')
        cy.urlNotContains(text)
        cy.urlContains(`/video-shows/${VIDEO_SHOW_ID}`)
        EPISODE_ID = text
      })
  })
  it('Edit episode', ()=>{
    ...
    cy.getCyVisibleClick('table-edit')
    cy.cardLoad()
    cy.get('.v-card-text').find('input').first().clear().type(`${EPISODE_TITLE}-edit`)
    cy.getCy('button-close').should('be.visible')
    cy.getCy('button-save').click()
    cy.alertMessage(ALERT_UPDATE)
    cy.urlContains(`/episodes/${EPISODE_ID}`)
    cy.getCyVisibleClick('button-close')
    cy.urlNotContains(`/episodes/${EPISODE_ID}`)
    cy.urlContains(`/video-shows/${VIDEO_SHOW_ID}`)
  })
})
