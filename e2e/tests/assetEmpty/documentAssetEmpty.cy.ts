...
describe(`Test document empty asset, Env: ${CY.cfg}`, { tags: ['@assetDocumentEmpty', '@assetEmpty'] }, () => {
  ...
  it('Test empty asset', () => {
    ...
    cy.get('.dam-image-detail__sidebar [data-cy="custom-field-title"] textarea').eq(0).type(`${ASSET_TITLE}`)
    cy.getCy('button-save').should('be.visible').click()

    cy.getCy('button-slots').should('be.visible').click()
    cy.circleLoad()
    cy.get('.sidebar-info__content')
      .invoke('text')
      .then((sidebarText) => {
    ...
    cy.get('.sidebar-info__content input[type="file"]', { timeout: 10000 })
      .eq(0)
      .selectFile(
        { contents: 'fixtures/document/sample.doc' },
    ...
    cy.api_getFileID().then((assetID) => {
      ...
      cy.circleLoad()
      cy.get('.dam-image-detail__sidebar [data-cy="custom-field-title"] textarea')
        .eq(0)
        .invoke('val')
        .then((assetTitle) => {
