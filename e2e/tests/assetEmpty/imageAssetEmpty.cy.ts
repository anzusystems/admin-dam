...
describe(`Test image empty asset, Env: ${CY.cfg}`, { tags: ['@assetImageEmpty', '@assetEmpty'] }, () => {
  ...
  it('Test empty asset', () => {
    ...
    cy.get('.dam-image-detail__sidebar [data-cy="custom-field-title"] textarea[rows]').type(`${ASSET_TITLE}`)
    ...
    cy.get('.sidebar-info__content')
      ...
      .then((sidebarText) => {
        ...
        cy.wrap(sidebarText).should(
          'include',
          'Náhľady sú určené pre hlavný obrázok v článku, ' +
    ...
    cy.get('.sidebar-info__content input[type="file"]', { timeout: 10000 })
      .eq(0)
      .selectFile({ contents: 'fixtures/image/sample.png' }, { waitForAnimations: true, force: true, action: 'select' })
