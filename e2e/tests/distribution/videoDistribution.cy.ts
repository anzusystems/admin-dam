...
import { CY, ASSET_TITLE, ASSET_DESCRIPTION, ALERT_CREATE } from '../../utils/common'
...
describe(`Test distribution Video function, Env: ${CY.cfg}`, { tags: ['@distributionVideo', '@distribution'] }, () => {
  ...
  it('Distribute video', () => {
    ...
    cy.get('textarea').eq(0).type(ASSET_TITLE)
    cy.get('textarea').eq(2).type(ASSET_DESCRIPTION)
    cy.get(':nth-child(3) > .v-col > .v-input > .v-input__control > .v-field > .v-field__field > .v-field__input').type(
      DISTRIBUTION_DATA.AUTHOR
    )
    cy.get('.v-card-actions > .bg-primary').click()
    cy.alertMessage(ALERT_CREATE)
    ...
    // Check status
    cy.get(':nth-child(1) > .text-body-2 > :nth-child(2) > .v-col > .v-chip > .v-chip__content', {
      timeout: 120000,
    ...
    cy.get(':nth-child(3) > .text-body-2 > :nth-child(2) > .v-col > .v-chip > .v-chip__content').should(
      'include.text',
      'Distribuovaný'
    ) // YT distribution
  ...
  it('Verify artemis', () => {
    ...
    cy.get('#sdc5d04ee76_title_baseTitle').invoke('val').should('eq', ASSET_TITLE)
  })
  it('Delete Test data', () => {
