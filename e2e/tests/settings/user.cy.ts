...
let USER_ID = ''
describe(`Test user function, Env: ${CY.cfg}`, { tags: ['@user', '@settings'], env: { visitBaseUrl: false } }, () => {
  it('Create user', () => {
    cy.visit('/settings')
    ...
    cy.get('.v-overlay .v-list-item').contains('DAM').click()
    cy.getCy('create-panel').click('top')
    cy.get('.v-overlay .v-chip__content').contains('DAM full')
