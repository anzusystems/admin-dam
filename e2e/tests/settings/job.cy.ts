...
describe(`Test job function, Env: ${CY.cfg}`, { tags: ['@job', '@settings'] }, () => {
  ...
  it('Test podcast synchronization JOB', () => {
    cy.request(`${CY.url.proto}://core-dam.${CY.url.domain}/api/adm/v1/asset/licence/100000/search?limit=10
      ...
      &distributedInServices=artemis_podcast_cms`).then((response) => {
      ...
      cy.get('[data-cy="custom-field-title"] textarea').first().clear({ force: true }).type(`${assetTitle}`)
      cy.get('[id="anzu-asset-detail-sidebar-actions"] [data-cy="button-save"]').click()
      cy.getCy('button-podcast').should('be.visible').click()
      cy.request(`${CY.url.proto}://core-dam.${CY.url.domain}/api/adm/v1/podcast/${podcastID}`).then((response) => {
        ...
        cy.contains('h4', 'Povolené na webe').parent().should('contain.text', 'nie')
        cy.contains('h4', 'Povolené v appke').parent().should('contain.text', 'nie')
        ...
        cy.alertMessage(ALERT_CREATE)
        cy.get(':nth-child(1) > :nth-child(2) > .v-chip > .v-chip__content').should(
          'include.text',
        ...
        cy.getCy('episode-list').click()
        cy.getCy('table-detail').eq(0).click()
        cy.contains('h4', 'Povolené na webe').parent().should('contain.text', 'áno')
        cy.contains('h4', 'Povolené v appke').parent().should('contain.text', 'áno')
      })
    })
  })
  it('Delete Test data', () => {
