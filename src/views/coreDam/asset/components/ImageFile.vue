...
<template>
  ...
  <VImg v-else :width="width" :height="width" :src="src">
    <template #placeholder />
  ...
  <div v-if="showActions">
    <VBtn variant="text" class="my-2 mr-2" @click.stop="dialog = true">
      {{ t('system.imagePreview.actions.selectImage') }}
    </VBtn>
    <VBtn v-if="fileIdModel !== null" variant="text" class="my-2" @click.stop="removeImage">
      {{ t('system.imagePreview.actions.unassign') }}
    </VBtn>
  </div>
  <VDialog v-if="showActions" v-model="dialog" :width="500">
    <VCard v-if="dialog" data-cy="delete-panel">
      <ADialogToolbar @on-cancel="onCancel">
        {{ t('system.imagePreview.actions.selectImage') }}
      </ADialogToolbar>
      <VCardText>
        <VTextField v-model="newFileId" :label="t('system.imagePreview.actions.selectImage')" />
      </VCardText>
      <VCardActions>
        <VSpacer />
