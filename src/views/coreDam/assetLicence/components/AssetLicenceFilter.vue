  ...
  <VForm name="search" @submit.prevent="submitFilter">
    <AFilterWrapper :touched="touched" @reset-filter="resetFilter">
      <VRow class="align-start">
        <VCol cols="1">
          <AFilterInteger v-model="filterAssetLicence.id" @update:model-value="onAnyFilterUpdate" />
        </VCol>
        <VCol cols="2">
          <AFilterString v-model="filterAssetLicence.extId" @update:model-value="onAnyFilterUpdate" />
        </VCol>
        <VCol cols="2">
