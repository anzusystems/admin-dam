...
<template>
  <VForm name="search" @submit.prevent="submitFilter">
    <AFilterWrapper :touched="touched" enable-advanced @reset-filter="resetFilter">
      <VRow class="align-start">
        <VCol cols="12" sm="2">
          <AFilterInteger v-model="filter.id" @update:model-value="onAnyFilterUpdate" />
        </VCol>
        <VCol cols="12" sm="5">
          <AFilterString v-model="filter.email" @update:model-value="onAnyFilterUpdate" />
        </VCol>
        <VCol cols="12" sm="5">
          <AFilterString v-model="filter.lastName" @update:model-value="onAnyFilterUpdate" />
        </VCol>
      </VRow>
      ...
      <template #advanced>
        <VRow>
          <VCol cols="12" sm="5">
            <PermissionGroupRemoteSelect v-model="filter.permissionGroups" @update:model-value="onAnyFilterUpdate" />
          </VCol>
        </VRow>
      </template>
    </AFilterWrapper>
